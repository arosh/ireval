...
def _ndcg_one_query(gs, rs, k):
    ...
    denom = sum(x / math.log(r + 1) for x, r in zip(gs_sorted, range(1, k + 1)))

    weight_dict = {}
    for w in gs:
        weight_dict[w.item] = w.weight

    rs_sorted = [weight_dict[w.item] for w in sorted(rs, key=lambda w: w.weight, reverse=True)]
    numer = sum(x / math.log(r + 1) for x, r in zip(rs_sorted, range(1, k + 1)))
